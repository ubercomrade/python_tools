...
def main():
    ...
    #venn3_unweighted
    venn3(subsets=np.around(np.array(frequency.iloc[0,:7]), -1), set_labels = (fname, sname, tname))
    ...
    first_model_sites = first_model_sites[['chromosome', 'start', 'end', 'name', 'score', 'strand', 'site']]
    first_model_sites.to_csv(out_dir + '/' + tag + '_all_first_model.sites', sep='\t', index=False, header=False)
    ...
    second_model_sites = second_model_sites[['chromosome', 'start', 'end', 'name', 'score', 'strand', 'site']]
    second_model_sites.to_csv(out_dir + '/' + tag + '_all_second_model.sites', sep='\t', index=False, header=False)
    ...
    third_model_sites = third_model_sites[['chromosome', 'start', 'end', 'name', 'score', 'strand', 'site']]
    third_model_sites.to_csv(out_dir + '/' + tag + '_all_third_model.sites', sep='\t', index=False, header=False)
