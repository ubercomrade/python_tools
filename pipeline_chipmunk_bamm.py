...
def pipeline_chipmunk_bamm(bed_path, bigwig_path, training_sample_size, testing_sample_size, shoulder,
    ...
    if not os.path.isfile(motifs + '/' + tag + '_motif_1.ihbcp'):
        ...
        print('File {0} already exists'.format(tag + '_motif_1.ihbcp'))
    ...
    args = ['python3', path_to_python_tools + 'compare_sites.py',
            ...
            '-b', scan + '/' + tag + '_BAMM_' + str(testing_sample_size) +'_' + str(fpr_for_thr) + '.bed',
...
def parse_args():
    ...
    parser.add_argument('-m', '--model_order', action='store', type=int, dest='model_order',
