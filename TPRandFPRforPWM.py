...
def main(fileInput, everyStr, homology, kind):
    ...
    results = {'FPR': [], 'TPR': [], 'Score': [], 'NormScore': []}
    for normValue in (round(k * 0.0005, 3) for k in range(1500, 2001, 1)):
        subMotifs = list(allMotifs)
        testMotifs = random.sample(subMotifs, k=round(0.3*len(allMotifs)))
        ...
        minS = minScore(PWM)
        maxS = maxScore(PWM)
        scoreValue = toScore(normValue, minScore=minS, maxScore=maxS)

        TPR = scanSequences(testMotifs, PWM, scoreValue, kind=kind)/len(testMotifs)
        if TPR == 0:
        ...
        while FPR <= 150:
            ...
            FPR += scanSequences(randomMotifs, PWM, scoreValue, kind=kind)
            if totalLen >= 150000:
                break
        if FPR == 0:
            FPR = 1.0/(totalLen)
        ...
        results['Score'].append(scoreValue)
        results['NormScore'].append(normValue)
    return(results)


if __name__ == '__main__':
    fileInput = '/Users/anton/Documents/Python/HOCOMOCO/FOXA2_HUMAN.H11MO.0.A.words'
    parser = argparse.ArgumentParser()
    parser.add_argument('-i', '--input', action='store',
                        dest='fileInput', help='Path to file with motifs')
    parser.add_argument('-o', '--output', action='store',
                        dest='fileOutput',
                        help='Path to file with results of calculation')
    parser.add_argument('-f', '--fasta', action='store_false', dest='fileType',
                        help='Type of input, if file is fasta or file contains [>] use this flag')
    parser.add_argument('-t', '--type', action='store', dest='kind',
    ...
    output = main(fileInput=fileInput, everyStr=everyStr, homology=homology,
                  kind=kind)

    with open(fileOutput, 'w', newline='') as csvfile:
        fieldnames = ['FPR', 'TPR', 'Score', 'NormScore']
