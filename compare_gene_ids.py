...
def main():
    ...
    plt.savefig(out_dir + '/IDs_COMPARE_PIC.png', dpi=150)
    ...
    write_ids(data=only_fm_ids,
              path=out_dir + '/only.{0}.ids.txt'.format(fname))
    write_ids(data=only_sm_ids,
              path=out_dir + '/only.{0}.ids.txt'.format(sname))
    write_ids(data=only_tm_ids,
              path=out_dir + '/only.{0}.ids.txt'.format(tname))
