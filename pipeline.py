...
def work_with_tf(bed_path, wig_path, fpr_for_thr, path_to_out):
    ...
    main_out = path_to_out + '/' + os.path.basename(bed_path).split('.')
    os.mkdir(main_out)
    ...
    tag = os.path.basename(bed_path).split('.')
    ...
    args = ['python3', '/home/anton/Programs/python_tools/calculate_roc.py', 'tpr',
            '-f', motifs + '/' + tag + str(motif_length) + '.fasta',
            '-i', '10',
            '-t', '300',
            '-v', '0.0005']
    ...
    for motif_length in range(11, 21):
        ...
        if st.ttest_ind(optimal, tmp)[1] < 0.001 and np.mean(tmp) > np.mean(optimal):
            optimal = tmp[:]
